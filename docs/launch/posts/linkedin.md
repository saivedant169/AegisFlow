...
Operationally it's a single Go binary, local-first, Apache-2.0, and runs without paid cloud services for the core. The governance decision itself adds single-digit microseconds; the gateway sustains 58,000+ requests/sec at ~1.1 ms p50, with roughly 80% test coverage. It's pre-1.0 (v0.8.0) — worth evaluating now, not yet a 1.0 guarantee.
